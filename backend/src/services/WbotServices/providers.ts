    ...
    if (urlmkauth != "" && Client_Id != "" && Client_Secret != "") {
      if (isNumeric(numberCPFCNPJ) === true) {
        if (cpfcnpj.length > 2) {
          ...
          if (isCPFCNPJ) {
            const textMessage = {
              text: formatBody(`¡Espera! ¡Estamos consultando en la base de datos!`, contact),
            };
            try {
              await sleep(2000)
                ...
                axios.request(config as any)
                  .then(async function (response) {
                    if (response.data == 'NULL') {
                      const textMessage = {
                        text: formatBody(`¡Registro no localizado! *CPF/CNPJ* incorrecto o inválido. ¡Intenta nuevamente!`, contact),
                      };
                      try {
                        await sleep(2000)
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, textMessage);
                      ...
                      try {
                        const textMessage = { text: formatBody(`¡Encontré tu registro! *${nome}* solo un momento por favor!`, contact) };
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, textMessage);
                        const bodyBoleto = { text: formatBody(`Aquí está la segunda copia de tu Factura!\n\n*Nombre:* ${nome}\n*Valor:* R$ ${valorCorrigido}\n*Fecha de Vencimiento:* ${anoMesDia}\n*Link:* ${urlmkauth}/boleto/21boleto.php?titulo=${titulo}\n\nEnviaré el *código de barras* en el siguiente mensaje para que te sea más fácil copiarlo!`, contact) };
                        await sleep(2000)
                        ...
                        if (bloqueado === 'sim') {
                          const bodyBloqueio = { text: formatBody(`${nome} vi tambien que tu conexión está bloqueada! Voy a desbloquearla para ti por *48 horas*.`, contact) };
                          await sleep(2000)
                          await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyBloqueio);
                          const bodyqrcode = { text: formatBody(`Estoy liberando tu acceso. Por favor espera!`, contact) };
                          await sleep(2000)
                          await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyqrcode);
                          var optionsdesbloq = {
                            method: 'GET',
                          ...
                          axios.request(optionsdesbloq as any).then(async function (response) {
                            const bodyLiberado = { text: formatBody(`¡Listo, lo liberé! Necesito que *desconectes* tu equipo de la corriente.\n\n*OBS: Solo desconéctalo.* \nEspera 1 minuto y vuelve a conectarlo!`, contact) };
                            await sleep(2000)
                            await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyLiberado);
                            const bodyqrcode = { text: formatBody(`¡Revisa si tu acceso ha vuelto! Si no ha vuelto, contacta con un atendente!`, contact) };
                            await sleep(2000)
                            await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyqrcode);
                          }).catch(async function (error) {
                            const bodyfinaliza = { text: formatBody(`¡Ups! ¡Algo salió mal! Digita *#* para volver al menú anterior y habla con un atendente!`, contact) };
                            await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyfinaliza);
                          });
                        }


                        const bodyfinaliza = { text: formatBody(`¡Estamos finalizando esta conversación! Si necesitas algo más, contáctanos!`, contact) };
                        await sleep(12000)
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyfinaliza);
                  ...
                  .catch(async function (error) {
                    try {
                      const bodyBoleto = { text: formatBody(`No pude encontrar tu registro.\n\nPor favor intenta nuevamente!\nO digita *#* para volver al *Menú Anterior*`, contact) };
                      await sleep(2000)
                      await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyBoleto);
                    } catch (error) {
          ...
          if (isCPFCNPJ) {
            ...
            axios.request(optionsc as any).then(async function (response) {
              ...
              if (totalCount === 0) {
                ...
                const body = {
                  text: formatBody(`¡Encontré tu registro! \n*${nome}* solo un momento por favor!`, contact),
                };
                await sleep(2000)
                await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, body);
                ...
                axios.request(optionsListpaymentOVERDUE as any).then(async function (response) {
                  ...
                  if (totalCount_overdue === 0) {
                    ...
                    axios.request(optionsPENDING as any).then(async function (response) {
                      ...
                      value_pending_corrigida = value_pending.toLocaleString('es', { style: 'currency', currency: 'BRL' });

                      const bodyBoleto = {
                        text: formatBody(`Aquí está la segunda copia de tu Factura!\n\n*Factura:* ${invoiceNumber_pending}\n*Nombre:* ${nome}\n*Valor:* R$ ${value_pending_corrigida}\n*Fecha de Vencimiento:* ${dueDate_pending_corrigida}\n*Descripción:*\n${description_pending}\n*Link:* ${invoiceUrl_pending}`, contact),
                      };
                      await sleep(2000)
                      await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyBoleto);
                      //GET DADOS PIX
                      ...
                      axios.request(optionsGetPIX as any).then(async function (response) {
                        ...
                        if (success === true) {
                          ...
                          axios.request(optionsBoletopend as any).then(async function (response) {
                            ...
                            if (response.data?.errors?.code !== 'invalid_action') {
                              ...
                              const bodyfinaliza = {
                                text: formatBody(`¡Estamos finalizando esta conversación! Si necesitas algo más, contáctanos!`, contact),
                              };
                              await sleep(2000)
                              await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyfinaliza);
                              await sleep(2000)
                        ...
                        const body = {
                          text: formatBody(`*¡Ups!*\n¡Ocurrió un error! Digita *#* y habla con un *Atendente*!`, contact),
                        };
                        await sleep(2000)
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, body);
                      });
                    ...
                    value_overdue_corrigida = value_overdue.toLocaleString('es', { style: 'currency', currency: 'BRL' });
                    const body = {
                      text: formatBody(`¡Tienes *${totalCount_overdue}* factura(s) vencida(s)! \nTe las enviaré. ¡Por favor espera!`, contact),
                    };
                    await sleep(2000)
                    await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, body);
                    const bodyBoleto = {
              ...
              if (response.data.type === 'error') {
                ...
                axios.request(boleto as any).then(async function (response) {
                  ...
                  axios.request(optionsPix as any).then(async function (response) {
                    ...
                    if (tipo === 'success') {
                      const bodyBoletoPix = {
                        text: formatBody(`Aquí está la segunda copia de tu Factura!\n\n*Factura:* ${idboleto}\n*Nombre:* ${nome}\n*Valor:* R$ ${valorCorrigido}\n*Fecha de Vencimiento:* ${datavencCorrigida}\n\nTe enviaré el *Código de Barras* y el *PIX* solo tienes que hacer clic en el que quieras utilizar y se copiará automáticamente! Luego realiza el pago en tu banco.`, contact),
                      };
                      ...
                      const body_pixqr = {
                        text: formatBody("QR CODE del *PIX*", contact),
                      };
                      await sleep(2000)
                      ...
                      axios.request(optionscontrato as any).then(async function (response) {
                        ...
                        if (status_internet !== 'A') {
                          const bodyPdf = {
                            text: formatBody(`*${nome}* vi tambien que tu conexión está bloqueada! Voy a desbloquearla para ti.`, contact),
                          };
                          await sleep(2000)
                          await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyPdf);
                          const bodyqrcode = {
                          ...
                          axios.request(optionsdesbloqeuio as any).then(async function (response) {
                            ...
                            if (tipo === 'sucesso') {
                              ...
                              axios.request(optionsRadius as any).then(async function (response) {
                                ...
                                if (tipo === 'success') {
                                  ...
                                  const bodyPdf = {
                                    text: formatBody(`¡Hice los procedimientos de liberación! Ahora espera hasta 5 minutos y revisa si tu conexión ha vuelto! .\n\nSi no ha vuelto, contacta con un atendente!`, contact),
                                  };
                                  await sleep(2000)
                                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyPdf);
                                  const bodyfinaliza = {
                              ...
                              const bodyerroatendent = {
                                text: formatBody(`¡Digita *#* para volver al menú y habla con un atendente!`, contact),
                              };
                              await sleep(2000)
                            ...
                            const bodyerro = {
                              text: formatBody(`¡Ups! ¡Ocurrió un error, digita *#* y habla con un atendente!`, contact),
                            };
                            await sleep(2000)
                            await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyerro);
                          });
                      ...
                      const bodyBoleto = {
                        text: formatBody(`Aquí está la segunda copia de tu Factura!\n\n*Factura:* ${idboleto}\n*Nombre:* ${nome}\n*Valor:* R$ ${valorCorrigido}\n*Fecha de Vencimiento:* ${datavencCorrigida}\n\nHaz clic aquí abajo en el código de barras para copiarlo, luego realiza el pago en tu banco!`, contact),
                      };
                      await sleep(2000)
                      await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyBoleto);
                      const body = {
                            ...
                            if (tipo === 'sucesso') {
                              ...
                              axios.request(optionsRadius as any).then(async function (response) {
                                ...
                                if (tipo === 'success') {
                                  ...
                                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, body_mensagem);
                                  const bodyPdf = {
                                    text: formatBody(`Necesito que *desconectes* tu equipo de la corriente.\n\n*OBS: Solo desconéctalo.* \nEspera 1 minuto y vuelve a conectarlo!`, contact),
                                  };
                                  await sleep(2000)
                                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyPdf);
                                  const bodyqrcode = {
                                    text: formatBody(`¡Revisa si tu acceso ha vuelto! Si no ha vuelto, contacta con un atendente!`, contact),
                                  };
                                  await sleep(2000)
                                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyqrcode);
                                  const bodyfinaliza = {
                              ...
                              const bodyerro = {
                                text: formatBody(`¡Ups! ¡Ocurrió un error y no pude desbloquear! ¡Digita *#* y habla con un atendente!`, contact),
                              };
                              await sleep(2000)
                              await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyerro);
                            }
                ...
                axios.request(optionscontrato as any).then(async function (response) {
                    ...
                    axios.request(optionsdesbloqeuio as any).then(async function (response) {
                      ...
                      if (tipo === 'sucesso') {
                        ...
                        const bodyerroatendente = {
                          text: formatBody(`¡Digita *#* y habla con un atendente!`, contact),
                        };
                        await sleep(2000)
                  ...
                  text: formatBody(`Ops! Ocorreu um erro e nao consegui desbloquear! Digite *#* e fale com um atendente!`
                    ...
                    const bodysembloqueio = {
                      text: formatBody(`¡Tu conexión no está bloqueada! Si tienes dificultades para navegar, contacta con un atendente!`, contact),
                    };
                    await sleep(2000)
