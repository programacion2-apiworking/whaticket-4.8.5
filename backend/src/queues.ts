...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    Sentry.captureException(e);
    logger.info(`Error al intentar consultar la cita: ${schedule.id}`);
  }
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        for (let i = 0; i < contactData.length; i++) {
          ...
          logger.info(`Registro enviado a la cola de disparos: Campanha=${campaign.id};Contato=${contacts[i].name};delay=${delay}`);
        }
